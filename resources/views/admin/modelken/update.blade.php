...
<div class="card">
  ...
  <form action="{{ url('admin/modelken/' . $modelken->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
    @csrf
    @method('put')
    <div class="card-body">
      <div class="mb-3">
        <label class="form-label" for="nama_model">Nama Merek *</label>
        <input class="form-control" id="nama_model" name="nama_model" type="text" placeholder="masukan nama model"
          value="{{ old('nama_model', $modelken->nama_model) }}" />
      </div>
    </div>
